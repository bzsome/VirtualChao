...
public class VApp extends MultiDexApplication {
    ...
    public void onCreate() {
        ...
        virtualCore.initialize(new VirtualCore.VirtualInitializer() {
            ...
            public void onMainProcess() {
                ...
                startInstXposed(gApp);
                startInstQQHongbao(gApp);
                Build.DEVICE.set("DEVICE的值");
                // check for update
    ...
    public static void startInstXposed(VApp vApp) {
        boolean isXposedInstalled = VirtualCore.get().isAppInstalled(XPOSED_INSTALLER_PACKAGE);
        try {
            File oldXposedInstallerApk = vApp.getFileStreamPath("XposedInstaller_1_24.apk");
        ...
        if (!isXposedInstalled) {
            File xposedInstallerApk = vApp.getFileStreamPath("XposedInstaller_1_31.apk");
            if (!xposedInstallerApk.exists()) {
                InputStream input = null;
                OutputStream output = null;
                try {
                    input = vApp.getApplicationContext().getAssets().open("XposedInstaller_3.1.5.apk_");
    ...
    public static void startInstQQHongbao(VApp vApp) {
        final String INSTALLER_PACKAGE = "me.veryyoung.qq.luckymoney";
        boolean isXposedInstalled = VirtualCore.get().isAppInstalled(INSTALLER_PACKAGE);
        try {
            File oldXposedInstallerApk = vApp.getFileStreamPath("QQHongbaoInstaller_1_24.apk");
            if (oldXposedInstallerApk.exists()) {
                VirtualCore.get().uninstallPackage(INSTALLER_PACKAGE);
                oldXposedInstallerApk.delete();
                isXposedInstalled = false;
                Log.d(TAG, "remove xposed installer success!");
            }
        ...
        if (!isXposedInstalled) {
            File xposedInstallerApk = vApp.getFileStreamPath("QQHongbaoInstaller_1_31.apk");
            if (!xposedInstallerApk.exists()) {
                InputStream input = null;
                OutputStream output = null;
                try {
                    input = vApp.getApplicationContext().getAssets().open("QQHongbao_1.1.4x.apk_");
                    output = new FileOutputStream(xposedInstallerApk);
                    byte[] buffer = new byte[1024];
                    int length;
                    while ((length = input.read(buffer)) > 0) {
